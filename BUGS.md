...
**(3) Complementação (2026-06-03, descoberta em teste ao vivo):** o reuso de (2) evitava o spawn duplicado mas **não religava o encaminhamento** — o `v1_embeddings_proxy` ainda devolvia **503 "embed-server não está ativo"** porque gateava por `embed_proc_active()` (handle interno, que continuava None) com um embed-server **saudável e servindo** na 8082. Sintoma confirmado: AKASHA logava `embed_text: HTTP 503` em looping com o embed-server up (em CPU). Fix: o gate do proxy passou a usar a **saúde** (`embed_health_ok(client, embed_health_port)`) — só devolve 503 se NÃO houver handle E o servidor não responder `/health`; se está saudável, encaminha. Usa `embed_health_port` (configurável: 8082 em produção, porta isolada em testes) para não quebrar o isolamento de teste.
...
#### Fix aplicado
**Aplicado (1ª camada, 2026-06-03 — pendente de validação no CachyOS):** o default de `embed_n_gpu_layers` foi mudado de **-1 (GPU) → 0 (CPU)** em `logos.rs` (nova const `DEFAULT_EMBED_N_GPU_LAYERS = 0`, usada no construtor e nas fixtures de teste). Tirar o embed-server (bge-m3, ~0,6 GB) da VRAM remove a causa raiz do churn: se ele não ocupa VRAM, o watchdog de VRAM nunca o mata, e a VRAM fica reservada aos LLMs de chat. É a saída escolhida pela usuária (o PC principal não indexa no Windows — onde CPU seria problemático por falta de AVX2 — então CPU por padrão é seguro no uso real). Testes Rust: `embed_n_gpu_layers_defaults_to_cpu`, `default_embed_n_gpu_layers_is_cpu` (default → `--n-gpu-layers 0`). **Validação pendente na máquina real** (ver 1º item do TODO): VRAM liberada (~0,6 GB esperado) e throughput de embedding em CPU no Ryzen. **Atenção:** valor explícito em `ecosystem.json` prevalece sobre o default — se estiver -1, trocar para 0.

Direções candidatas restantes (hardening da política de VRAM — **não implementadas**, separadas do embed):
- **(E — recomendada, alinhada à diretiva) Não matar modelos P3 no limiar normal (85%):** o watchdog de VRAM, ao bloquear a 85%, deve apenas setar `p3_vram_blocked=true` (delay loop já cuida de atrasar P3) e **não** chamar `do_silence()`. A descarga de modelos fica reservada ao limiar extremo de crash-prevention (VRAM > 97% / RAM crítica). Resolve a violação de diretiva (vale também para os servidores de chat, que o embed-em-CPU não cobre). Itens no TODO.
...
- **Data:** 2026-06-17
- **App(s):** KOSMOS + HUB
- **Componente:** `KOSMOS/app/utils/paths.py`, `KOSMOS/app/core/database.py`, novo `sources_backup.py`; `HUB/src-tauri/src/commands/backup.rs` (pendente)
- **Commit do fix:** lado KOSMOS pendente de commit; lado HUB não iniciado
- **Descoberta via:** uso real — a usuária notou que a lista de fontes do KOSMOS "sumiu" e apontou que deveria estar na pasta sincronizada (com backup) e visível na aba Fontes do HUB
- **Tempo de diagnóstico:** ~40 min

#### Ambiente
- **Máquina(s) afetadas:** todas
- **OS:** Linux/Windows
- **Modo:** produção
- **Reproduzível em:** qualquer máquina (o banco sempre nasceu local)

#### Sintoma observado
A lista de fontes (feeds) do KOSMOS não sobrevive à perda/reset do banco local, não sincroniza entre máquinas e não aparece na aba "Fontes" do HUB (que existe). O `sources.json` em `.backup/kosmos/` ficava `[]`.

#### Causa raiz
1. **Banco local, não sincronizado:** `paths.py` definia `DB_PATH = LOCAL_DATA_DIR/"kosmos.db"` (`~/.local/share/kosmos/`), contrariando a docstring do `database.py` e o `CLAUDE.md` ("Banco SQLite sincronizado via Syncthing, `sync_root/kosmos/`"). O log foi movido ao sync_root no BUG-038, mas o banco ficou para trás.
2. **Backup do HUB com schema antigo:** `backup.rs::backup_kosmos_feeds` consulta `SELECT url, name, feed_type, active FROM feeds` — colunas do KOSMOS pré-v3. O schema v3 usa `title`/`category`/`enabled` → a query falhava/retornava vazio. A integração HUB↔KOSMOS (backup, restore, Fontes) nunca foi atualizada para o v3.

#### Impacto
Risco de **perda total das fontes** da usuária (sem sync nem backup efetivo) e ausência da visão unificada de fontes no HUB.

#### Fix aplicado (lado KOSMOS — FEITO)
- `paths.py`: `_resolve_db_path()` resolve `{sync_root}/kosmos/kosmos.db` (com `mkdir`), fallback local sem sync_root. `LEGACY_LOCAL_DB` exposto.
- `database.py`: `_migrate_local_db_to_sync()` move o banco local antigo para o sync_root (uma vez), guardado por `DB_PATH == {sync_root}/kosmos/kosmos.db` (testes com tmp não disparam). `restore_sources_if_empty()` chamado na init (só no banco real) restaura as fontes do `sources.json` se a tabela estiver vazia.
- novo `core/sources_backup.py`: `export_sources()` grava as fontes (formato v3 `{url,title,category,enabled}`) em DUAS cópias — `{sync_root}/kosmos/sources.json` e `{sync_root}/.backup/kosmos/sources.json`; `restore_sources_if_empty()` lê (preferindo a viva, caindo para o backup).
