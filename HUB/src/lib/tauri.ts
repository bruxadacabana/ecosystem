...
export const gitCommitForApp = (app: string): Promise<TauriResult<string>> =>
  call<string>('git_commit_for_app', { app })

export const gitScheduledCommit = (runningApps: string[]): Promise<TauriResult<string>> =>
  call<string>('git_scheduled_commit', { runningApps })
...
export const logosSetModelAssignment = (
  app: string,
  modelType: string,
  model: string,
): Promise<TauriResult<void>> =>
  call<void>('logos_set_model_assignment', { app, modelType, model })

export const logosGetRecommendedModels = (): Promise<TauriResult<RecommendedModel[]>> =>
  call<RecommendedModel[]>('logos_get_recommended_models')
