...
**Acesso via Tailscale (2026-06-18):** o T410 entrou na tailnet (conta `bruxadacabana@`) com nome **`t410`** e IP **`100.79.50.76`** (`tailscaled` habilitado via systemd, reconecta no boot; `tailscale0` na zona `trusted` do firewalld). Para o SearXNG ser alcançável pela tailnet (não só pela LAN), o bind foi mudado de `192.168.0.252` para **`0.0.0.0`** — no `searxng-docker`, o `docker-compose.yml` usa `${SEARXNG_HOST:+...}` e a variável fica no `/home/spacewitch/searxng/.env` (`SEARXNG_HOST=0.0.0.0`; era `192.168.0.252`). Então o SearXNG agora atende em **dois endereços**: `http://192.168.0.252:8080` (LAN, rápido em casa) e **`http://100.79.50.76:8080` / `http://t410:8080` (tailnet, alcançável de qualquer máquina/rede)**. `ecosystem.json` é **por-máquina** (não sincronizado): no PC principal, `akasha.web_search_backend_fallback = http://100.79.50.76:8080/` (tailnet) foi adicionado ao lado do `web_search_backend` (LAN). As **máquinas remotas** (work PC, laptop) precisam do Tailscale instalado + logado na mesma conta, e devem usar o endereço da tailnet (`http://100.79.50.76:8080` ou `http://t410:8080`) — como primário, já que a LAN `192.168.0.252` não as alcança fora de casa.

**Backend de busca web — fila + Tailscale + vendorizado local-only (2026-06-17):** a busca web do AKASHA usa uma **fila de disponibilidade** (`services/web_search.py` → `_active_searxng()`, probe `/healthz` paralelo, escolhe o de maior prioridade vivo): 1º **remoto** (`akasha.web_search_backend`) → 2º **local** (`akasha.web_search_backend_fallback`) → 3º **vendorizado** (`http://127.0.0.1:8889`, o HUB sobe sob demanda via watchdog em `commands/searxng.rs`). Marginalia sempre em paralelo; mwmbl condicional; DDG só último recurso. A página de busca tem banner (`GET /search/backend-status`) que sinaliza degradação/engines fora. **Remoto = T410** (Docker, 192.168.0.252:8080 na LAN; **e** `http://100.79.50.76:8080` / `http://t410:8080` pela **Tailscale**). Tailscale ativo (tailnet `bruxadacabana@`, MagicDNS on): `t410`=100.79.50.76 (SearXNG), `gallant-adm`=100.93.245.2 (work PC Windows), `thewitch`=100.125.205.75 (PC principal CachyOS). O work PC já aponta `web_search_backend` pro T410 via tailnet (confirmado: healthz 200 + ~50 resultados). **Pendente:** teste cross-network do laptop (hotspot do celular). Nota: o SearXNG precisava bindar em 0.0.0.0 (`SEARXNG_HOST=0.0.0.0` no `.env` do searxng-docker) para responder fora da LAN. **Vendorizado** = SearXNG nativo em Python sem Docker (roda no Windows; commit congelado `502c820`; stub `pwd` p/ Windows; `settings.base.yml` porta 8889/limiter off/json) — decisão de torná-lo **fallback local-only (3b)**: tirar a árvore pesada do repo (gitignore) e recriá-la pelo setup (clone do commit + patches em `vendor/patches/`); **pendente** implementar. Cross-device: interesses já são cross-device via `shared_topic_profile`; histórico bruto vai para `shared_history` em sync_root e alimenta **só a reflexão** (nunca pop-ups).
...
**Plano aprovado (2026-06-20) — AKASHA-ferramenta headless no servidor T410** (fases no TODO raiz "### AKASHA-ferramenta headless no servidor T410 …"; plano completo em `~/.claude/plans/clever-yawning-beaver.md`; memória [[project-akasha-on-t410]]). Mover a **ferramenta** (crawl + index + busca) para rodar **24/7 no T410** (sempre ligado, Xeon sem AVX). Decisões da usuária: **(1)** ferramenta inteira no T410 — `akasha.db` vive lá; a **busca abre com GUI completo via URL `http://t410:7071`**, acessada por **todos os dispositivos** pela Tailscale (AKASHA deixa de ser por-máquina, vira central como o SearXNG); razão técnica: a busca lê o MESMO `akasha.db` que o crawling constrói, então índice e busca ficam juntos no servidor. **(2)** Só a ferramenta no T410 — a **assistente** (persona/reflexões/insights) fica **desligada no servidor** (`akasha.assistant_enabled=false`) e roda **sob demanda no PC principal**. **(3)** Embeddings via Tailscale ao **proxy LOGOS `http://thewitch:7072`** (T410 sem AVX não embeda; não usar 8082 direto — preserva a serialização `embed_semaphore`); **FTS sempre, semântico só com o PC principal ligado** (fallback gracioso pra FTS, indicado no HUB). **(4)** O HUB **diferencia ferramenta ↔ assistente** no card da AKASHA e controla o crawling remoto (toggle pause/resume + status + RAM do servidor). **(5)** Gestão de RAM do T410: `MemoryHigh/Max` no `akasha.service` (cgroup; folga pro SearXNG co-hospedado) + earlyoom + endpoint `/system/memory` no HUB.

**Princípio permanente — a assistente processa o backlog ao abrir, em prioridade baixa:** quando a assistente é aberta, deve **processar todo o acervo já baixado pela ferramenta desde a última vez**, mas em **prioridade mais baixa que o tempo-real** (o que a usuária acessa no momento). Backfill da assistente = **P3** do LOGOS (nunca bloqueia, só atrasa); interação ao vivo = **P1**. Gerenciamento de fila é central.

**Recuperação de disco (T410 = disco único sem redundância):** o `akasha.db` (índice, pesado) **não** sincroniza — é **re-rastreável**. A **lista de fontes** (`crawl_sites`) é o seed leve que reconstrói tudo → exportada para `{sync_root}/akasha_seed/crawl_sites.json` (Syncthing, já instalado no T410) e auto-restaurada num disco novo. Runbook completo de instalação em `T410_SETUP.md` (versionado, sobrevive ao disco). **Pendência registrada:** as **fontes do KOSMOS** (feeds RSS, lista separada) merecem o mesmo backup no sync_root.
...
**Toda configuração deve estar na UI (2026-06-02):** "toda configuração de todos os programas do ecossistema deve ser exibida na UI". Nenhum valor de config pode ser editável **apenas** via JSON — cada chave precisa de controle de UI em algum lugar (UI própria do app OU HUB). Estado de runtime (`bg_processing.*`, `pending_insights`, `last_git_head`) e plumbing interno (paths derivados, ports internos) não contam como configuração do usuário. Item no TODO raiz (seção "Painel de configuração do ecossistema") cobre um **editor visual das seções do `ecosystem.json` no HUB**. **Implementado (2026-06-19, abordagem híbrida):** além dos painéis dedicados (SyncSetup=sync_root/credenciais, LOGOS, Busca=web_search_backend), a aba **Config → "Todas as configs"** (`HUB/src/views/ConfigView.tsx`, helper testável `src/lib/config-fields.ts`) é um editor **genérico** que lê o `ecosystem.json` (`read_ecosystem_config`) e expõe TODAS as chaves de config, escondendo runtime/plumbing (`bg_processing`, `incoming_insights`, `pending_insights`, `last_git_head`, `exe_path`, `base_url`, `config_path`); salva via `save_ecosystem_config` (merge por chave). À prova de chaves novas. `sync_root` fica read-only ali (editar via Sync, que deriva os paths). Motivo de o HUB precisar cobrir tudo: o `ecosystem.json` é por-máquina (em `%APPDATA%/ecosystem`, fora do sync_root, **não sincronizado**).

**Criptografia de segredos no `ecosystem.json` (2026-06-19, implementado — abordagem B):** senhas/chaves (`hub.syncthing_gui_password`, `akasha.marginalia_api_key`, `akasha.qbt_password`; detectadas por nome conter `password`/`api_key`/`token`/`secret` — `unpaywall_email` NÃO é segredo) são cifradas em repouso com **AES-256-GCM**. Valor = `enc:`+base64(`nonce[12]`||ciphertext||`tag[16]`); chave AES-256 em `{data_dir}/ecosystem/.secret.key` (nunca sincronizada/commitada). Formato **idêntico** nos dois módulos, interop provada por **vetor de teste conhecido**: `ecosystem_secrets.py` (raiz, Python, `cryptography>=43` no AKASHA) e `HUB/src-tauri/src/secrets.rs` (Rust, `aes-gcm`). O HUB cifra ao salvar (editor/credenciais), decifra ao ler p/ exibir, e migra texto puro→cifrado no startup (`migrate_plaintext_secrets`); a AKASHA decifra `marginalia_api_key` ao ler e cifra/decifra na Settings. `decrypt()` é no-op em valores não-`enc:`. **Threat model:** local/offline/sem conta → chave na mesma máquina; protege contra exposição **casual** (backup/screenshare/cópia), não contra acesso total à máquina. `atualizar.sh`/`.bat` cobrem o `cryptography` via `uv sync` do AKASHA; os crates Rust entram no `cargo tauri build` (os scripts não buildam Rust). Memórias: [[project-secrets-encryption]], [[project-hub-config-editor]].
