...
**Embed-server roda em CPU por padrão (decisão 2026-06-03, BUG-028):** o default de `logos.embed_n_gpu_layers` é **0 (CPU)**. O embed-server (bge-m3 Q8, ~567 MB / ~0,6 GB VRAM) é leve e roda em CPU sem disputar VRAM com os LLMs de chat de AKASHA+Mnemosyne (uso padrão do PC principal — os dois LLMs juntos já passam de 80% da VRAM da RX 6600). Manter o embedding fora da VRAM elimina pela raiz o BUG-028 (watchdog de VRAM matava o embed-server P3 ao passar do limite, e o watchdog do embed-server o ressuscitava → loop de restart → Mnemosyne dava timeout e não indexava). Configurável por máquina via `ecosystem.json` (ex.: -1 para forçar GPU). Pendente de validação na máquina real (ver primeiro item do TODO): medir VRAM liberada e throughput de embedding em CPU no Ryzen 5 4600G. **Atenção:** valor explícito no `ecosystem.json` prevalece sobre o default — se estiver setado -1, trocar para 0. **Questão correlata em aberto (BUG-028, não corrigida):** o watchdog de VRAM chama `do_silence()` (mata modelos P3) já no limiar normal de 85%, o que viola a diretiva "P3 nunca é bloqueado, apenas atrasado, exceto em situação extrema"; o threshold acordado é 93–95% (o caso padrão dual-server já usa >80%), e o bump condicional 85→93 existente não conta o embed-server. Ver itens no TODO.
