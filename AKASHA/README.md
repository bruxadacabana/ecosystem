...
>
> **Fila de disponibilidade (implementada):** o AKASHA tenta os SearXNG nesta ordem, usando o de maior prioridade que estiver **vivo** (probe `/healthz` em paralelo): 1º **remoto** (`web_search_backend`); 2º **local** (`web_search_backend_fallback`, ex.: `http://localhost:8080`); 3º **vendorizado** (porta 8889 — ver seção do SearXNG vendorizado). Ambas as URLs editáveis em **Settings**. A Marginalia roda **sempre em paralelo** ao SearXNG ativo; o DuckDuckGo é o último recurso só quando nada responde. A página de busca mostra um **banner** (auto-carregado via HTMX, `GET /search/backend-status`) quando o backend está degradado — "usando SearXNG vendorizado/local" ou "nenhum SearXNG disponível → Marginalia/DDG"; o banner some quando o backend de maior prioridade está servindo.
